...
def clean_papers(bot = None,mid = None,top = None):
    # bot = The bottom year t start taking citations from in the window of interest
    # mid1 = The top year to stop taking citations from in the window of interest
    # mid2 = The bottom year to start taking citations from in the giving window
    # top = The first year in which citations should stop being considered for giving window
    ...
    mid_lab = dt.datetime.strptime('{}-01-01'.format(mid2),'%Y-%m-%d')
            ...
            for row in reader:
                if row != '':
                    try:
                        ...
                        if paper_date.year == 2000:
                            print(paper_date.year)
                        if paper_date >= mid_lab and paper_date < top_lab and row[0] != '' and row[11] != '':
                            ...
                            cnt_give_period += 1
                        if paper_date >= bot_lab and paper_date < mid_lab and row[0] != '' and row[11] != '':
...
def clean_papersII():
    ...
    # Store receiving and giving separately in dicts
    df_period = dict.fromkeys(range(2000,2020))
    
    try:
        cnt_period,totalcnt = 0, 0
        with open('MAG_Untouched_Papers.txt', 'rb') as f:
            reader = csv.reader(codecs.iterdecode(f, 'utf-8'), delimiter='\t')
            for row in reader:
                try:
                    paper_date = dt.datetime.strptime(row[24],'%Y-%m-%d')
                    if row[0] != '' and row[11] != '' and row[0] != None and row[11] != None:
                        if df_period[paper_date.year] == None or df_period[paper_date.year] == None:
                            df_period[paper_date.year] = dict.fromkeys(["paper_id","journal_id"])
                            df_period[paper_date.year]["paper_id"] = [int(row[0])]
                            df_period[paper_date.year]["journal_id"] = [int(row[11])]
                        else:
                            df_period[paper_date.year]["paper_id"].append(int(row[0]))
                            df_period[paper_date.year]["journal_id"].append(int(row[11]))
                        cnt_period += 1
                except Exception as e:
                    pass
                ...
                if totalcnt % 1000000 == 0 and totalcnt != 0:
                    #Report progress
                    print("Giving citations period items found:" + str(cnt_period))
                    print("Items searched:" + str(totalcnt))
    
    except Exception as e:
        pass

    print("Final period items found:" + str(cnt_period))
    print("Items searched: "+str(totalcnt))

    for year in df_period.keys():
        if df_period[year] != None and df_period[year] != None:
            #store giving
            df = pd.DataFrame.from_dict(df_period[year])
            df.to_csv('{}_paper-journals.csv'.format(year), index=False)

    for year in df_period.keys():
        if df_period[year] != None and df_period[year] != None:
            clean_journals()
            clean_refs(year,year)
            translate_refs(year,year)
...
def test_m(b,m):
    for year in range(int(b),int(m)):
        ...
        if year == 2010 or year == 2011:
            ...
            #edges.to_csv("raw-edges-{}.csv".format(year), sep="\t")
